...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Password Generator', icon: '🔑' });
    ...
    function generatePassword() {
        ...
        if (length < 4 || length > 128) {
            outputEl.textContent = 'Error: Length must be between 4 and 128';
            return;
        }
        ...
        if (!useUpper && !useLower && !useNumbers && !useSymbols) {
            outputEl.textContent = 'Error: Select at least one character type';
            return;
        }
