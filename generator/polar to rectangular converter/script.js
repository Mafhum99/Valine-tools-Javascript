...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Polar to Rectangular Converter', icon: '📐' });
    ...
    function calculate() {
        ...
        steps += 'Step 2: Calculate x\n';
        steps += `  x = ${formatNum(r)} × cos(${formatNum(thetaRad)})\n`;
        steps += `  x = ${formatNum(x)}\n\n`;

        steps += 'Step 3: Calculate y\n';
        steps += `  y = ${formatNum(r)} × sin(${formatNum(thetaRad)})\n`;
        steps += `  y = ${formatNum(y)}\n\n`;

        steps += 'Result:\n';
        steps += `  Rectangular coordinates: (${formatNum(x)}, ${formatNum(y)})\n`;
        steps += `  (x, y) = (${formatNum(x)}, ${formatNum(y)})`;
