        ...
        try {
            ...
            if (ipv4Match) {
                ...
                const isPrivate = (octets[0] === 10) ||
                    ...
                    (octets[0] === 127);
                const ipClass = octets[0] < 128 ? 'A' : octets[0] < 192 ? 'B' : octets[0] < 224 ? 'C' : octets[0] < 240 ? 'D' : 'E';
                result = `IP Address: ${input}\nType: IPv4\nClass: ${ipClass}\nBinary: ${binary}\nHex: ${hex}\nPrivate/Local: ${isPrivate ? 'Yes' : 'No'}\nLoopback: ${octets[0] === 127 ? 'Yes' : 'No'}`;
            } else if (input.match(ipv6Regex)) {
                const expanded = input.split(':').map(g => g.padStart(4, '0')).join(':');
                result = `IP Address: ${input}\nType: IPv6\nExpanded: ${expanded}\nCompressed: ${input}`;
            } else {
