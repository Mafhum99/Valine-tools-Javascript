...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'IP Subnet Calculator', icon: '🌐' });
    ...
    const cidr = parseInt($('#cidr').value);
    ...
    if (cidr < 1 || cidr > 32) {
      outputEl.textContent = 'CIDR must be 1-32';
      return;
    }
    ...
    const firstUsable = cidr >= 31 ? 'N/A' : toIP(network + 1);
    const lastUsable = cidr >= 31 ? 'N/A' : toIP(broadcast - 1);
    
    outputEl.textContent = 'IP Address: ' + ip + '/' + cidr + '\n\n' +
