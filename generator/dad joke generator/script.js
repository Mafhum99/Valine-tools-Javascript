...
/**
 * Dad Joke Generator - 623
 * Generate dad jokes
 */

document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Dad Joke Generator', icon: '👨' });
    ...
    const dadJokes = [
        ...
        { setup: "Why do seagulls fly over the ocean?", punchline: "Because if they flew over the bay, we'd call them bagels!" },
        ...
        { setup: "What did the grape do when he got stepped on?", punchline: "Nothing, he just let out a little wine." },
        ...
        { setup: "I used to be addicted to the hokey pokey...", punchline: "But then I turned myself around." },
        ...
        { setup: "Where do boats go when they're sick?", punchline: "To the boat doc!" },
        { setup: "Why did the scarecrow win an award?", punchline: "He was outstanding in his field!" },
        { setup: "I just got a job at a place that makes chess pieces.", punchline: "It's mostly knight shifts." },
    ...
    function generateDadJokes(topic) {
        const jokeId = `DJ-${Date.now().toString(36).toUpperCase()}`;
        const count = 5;

        let output = `DAD JOKE GENERATOR\n`;
        output += `${'='.repeat(50)}\n`;
        output += `Theme: ${topic || 'Classic Dad'}\n`;
        output += `ID: ${jokeId}\n`;
        output += `${'='.repeat(50)}\n\n`;

        output += `"Hi ${topic || 'Champ'}, I've got some jokes for you!"\n\n`;
        output += `${'-'.repeat(50)}\n\n`;
        ...
        for (let i = 0; i < count; i++) {
            ...
            let attempts = 0;
            do {
                joke = randomChoice(dadJokes);
                attempts++;
            } while (usedJokes.has(joke.setup) && attempts < 100);
            usedJokes.add(joke.setup);

            output += `DAD JOKE ${i + 1}:\n`;
            output += `${'-'.repeat(30)}\n`;
            output += `Dad: ${joke.setup}\n`;
            output += `Kid: "I don't know, what?"\n`;
            output += `Dad: ${joke.punchline} 😄\n\n`;
        }

        output += `${'-'.repeat(50)}\n`;
        output += `"Come on, those were GOLD!" — Every Dad, Ever\n\n`;
        output += `${'='.repeat(50)}\n`;
        output += `Dad Joke ID: ${jokeId}\n`;
        output += `End of Dad Jokes - ${jokeId}\n`;
    ...
    function calculate() {
        ...
        try {
            outputEl.textContent = generateDadJokes(input);
        } catch (error) {
