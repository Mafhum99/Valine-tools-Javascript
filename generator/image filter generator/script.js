...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Image Filter Generator', icon: '' });

    const imageUpload = $('#image-upload');
    const filterControls = $('#filter-controls');
    const noImageMsg = $('#no-image-message');
    ...
    const filters = {
        ...
        'hue-rotate': { el: $('#hue-rotate'), val: 0, unit: 'deg', displayEl: $('#hue-rotate-value') },
    ...
    function generateFilter() {
        ...
        try {
            ...
            for (const [name, cfg] of Object.entries(filters)) {
                ...
                if (value !== cfg.val) {
                    filterParts.push(`${name}(${value}${cfg.unit})`);
                }
            }
    ...
    function reset() {
        for (const [, cfg] of Object.entries(filters)) {
    ...
    Object.values(filters).forEach(cfg => {
