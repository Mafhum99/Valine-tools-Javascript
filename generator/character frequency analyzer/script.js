...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function calculate() {
        ...
        try {
            ...
            // Display visible characters with special handling
            ...
            const lines = sorted.map(([char, count]) => {
                const pct = ((count / total) * 100).toFixed(1);
                const display = displayChar(char);
                const bar = '#'.repeat(Math.round((count / sorted[0][1]) * 20));
                return `${display.padEnd(10)} ${String(count).padStart(5)}  ${pct.padStart(6)}%  ${bar}`;
            });

            const result = `Total characters: ${total}\nUnique characters: ${sorted.length}\n\nChar       Count     Percent  Graph\n${'-'.repeat(55)}\n` + lines.join('\n');
