 ...
 * Reverse: HTML tags to MD syntax
 */

document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'HTML to Markdown Converter', icon: '\u{1F4DD}' });
    ...
    function htmlToMarkdown(html) {
        ...
        // Remove DOCTYPE, html, head, body, script, style tags
        ...
        md = md.replace(/<li[^>]*>([\s\S]*?)<\/li>/gi, () => {
            counter++;
            // This won't work with replace callback, handle below
            return '<LI>' + arguments[1] + '</LI>';
        });
        md = md.replace(/<LI>([\s\S]*?)<\/LI>/g, () => {
            counter++;
    ...
    function execute() {
        ...
        try {
            outputEl.textContent = htmlToMarkdown(input);
        } catch (e) {
