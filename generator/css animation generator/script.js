...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'CSS Animation Generator', icon: '\u{2728}' });
    ...
    function generate() {
        ...
        styleEl.textContent = keyframes[type];
        document.head.appendChild(styleEl);

        const name = animationNames[type];
        const css = `${keyframes[type]}

.element {
    animation: ${name} ${dur} ${del} ${iter};
}`;

        preview.style.animation = 'none';
        preview.offsetHeight; // force reflow
        preview.style.animation = `${name} ${dur} ${del} ${iter}`;
