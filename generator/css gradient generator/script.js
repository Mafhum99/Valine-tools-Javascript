 ...
 * Generate linear and radial CSS gradients
 */

document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'CSS Gradient Generator', icon: '' });
    ...
    function generateGradient() {
        try {
            ...
            let css, gradientValue;

            if (type === 'linear') {
                const angle = angleSlider.value;
                angleValue.textContent = angle + 'deg';
                gradientValue = `linear-gradient(${angle}deg, ${c1}, ${c2})`;
            } else {
    ...
    // Toggle angle visibility based on gradient type
    gradientType.addEventListener('change', () => {
        angleGroup.style.display = gradientType.value === 'linear' ? '' : 'none';
        generateGradient();
    });
