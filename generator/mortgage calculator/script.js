...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Mortgage Calculator', icon: '🏠' });
    ...
    const homePrice = parseFloat(inputEl.value) || 0;
    const downPayment = parseFloat($('#downPayment').value) || 0;
    const annualRate = parseFloat($('#rate').value) || 0;
    const years = parseInt($('#term').value) || 30;
    
    if (homePrice <= 0) { outputEl.textContent = 'Please enter home price'; return; }
    ...
    outputEl.textContent = 'Home Price: $' + homePrice.toLocaleString() + '\n' +
