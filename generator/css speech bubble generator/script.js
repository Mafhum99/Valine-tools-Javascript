 ...
 * Generate CSS speech bubbles with tails
 */

document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'CSS Speech Bubble Generator', icon: '\u{1F4AC}' });

    const bgColor = $('#bgColor');
    const bgColorText = $('#bgColorText');
    const textColor = $('#textColor');
    const textColorText = $('#textColorText');
    const tailPosition = $('#tailPosition');
    ...
    function generate() {
        ...
        const pos = tailPosition.value;

        if (styleEl) { styleEl.remove(); styleEl = null; }
        styleEl = document.createElement('style');

        let bubbleStyles = `background: ${bg}; color: ${tc};`;
        let tailStyles = '';

        if (pos === 'bottom') {
            bubbleStyles += ' margin-bottom: 20px;';
            tailStyles = `
                bottom: -20px;
                left: 30px;
            ...
            bubbleStyles += ' margin-right: 20px;';
            tailStyles = `
                right: -20px;
                top: 50%;
                transform: translateY(-50%);
...
.speech-bubble::after {
    ...
    function clear() {
        ...
        tailPosition.value = 'bottom';
        if (styleEl) { styleEl.remove(); styleEl = null; }
        output.textContent = '-';
    }

    syncColor();
    [bgColor, bgColorText, textColor, textColorText, tailPosition].forEach(el => {
