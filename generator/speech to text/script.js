...
/**
 * Speech to Text
 * Convert spoken words to text using Web Speech API
 */

document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Speech to Text', icon: '🎙️' });
    ...
    recognition.onresult = (event) => {
        ...
        let html = '';
        if (finalTranscript) {
            html += `<p class="final-text">${escapeHtml(finalTranscript)}</p>`;
        }
        if (interimTranscript) {
            html += `<p class="interim-text">${escapeHtml(interimTranscript)}</p>`;
        }
    ...
    recognition.onend = () => {
        statusEl.textContent = finalTranscript
            ? `Done. ${finalTranscript.trim().split(/\s+/).length} words captured.`
    ...
    function startListening() {
        ...
        try {
            ...
            showToast('Could not start speech recognition: ' + e.message);
        }
    }
    ...
    function clearResult() {
        finalTranscript = '';
        outputEl.innerHTML = '';
        statusEl.textContent = 'Ready - Click "Start Listening"';
        try {
            recognition.stop();
        } catch (e) { /* ignore */ }
        startBtn.disabled = false;
