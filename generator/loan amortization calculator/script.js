...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Loan Amortization Calculator', icon: '🏦' });
    ...
    if (principal <= 0) { outputEl.textContent = 'Please enter loan amount'; return; }
    ...
    let schedule = 'Loan: $' + principal.toLocaleString() + '\n' +
      'Rate: ' + annualRate + '%\n' +
      'Term: ' + years + ' year(s)\n\n' +
    ...
    for (let i = 1; i <= Math.min(12, numPayments); i++) {
      ...
      schedule += 'Month ' + i + ': Principal $' + principalPaid.toFixed(2) + ', Interest $' + interest.toFixed(2) + ', Balance $' + Math.max(0, balance).toFixed(2) + '\n';
    }
