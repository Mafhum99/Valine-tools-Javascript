...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Color Harmonizer', icon: '' });

    const base = $('#ch-base'), baseText = $('#ch-base-text');
    const harmony = $('#ch-harmony');
    ...
    function gen() {
        ...
        switch (type) {
            ...
            case 'split-complementary': hues = [hsl.h, (hsl.h + 150) % 360, (hsl.h + 210) % 360]; break;
            case 'tetradic': hues = [hsl.h, (hsl.h + 90) % 360, (hsl.h + 180) % 360, (hsl.h + 270) % 360]; break;
        }

        const colors = hues.map(h => {
            const rgb = Color.hslToRgb(h, hsl.s, hsl.l);
        ...
        colors.forEach((c, i) => {
            css += `  --color-${names[i] || i}: ${c};\n`;
            pHtml += `<div style="flex:1;min-width:60px;height:80px;background:${c};border-radius:8px;display:flex;align-items:flex-end;justify-content:center;padding:6px;">
                <span style="font-size:10px;color:#fff;background:rgba(0,0,0,0.5);padding:2px 6px;border-radius:4px;font-family:monospace;">${c}</span>
            </div>`;
        });
        css += '}';
    ...
    sync(base, baseText);
    harmony.addEventListener('change', gen);
