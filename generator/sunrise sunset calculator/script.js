...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Sunrise/Sunset Calculator', icon: '🌅' });
    ...
    if (isNaN(date)) { outputEl.textContent = 'Please select date'; return; }
    if (lat < -90 || lat > 90) { outputEl.textContent = 'Latitude must be -90 to 90'; return; }
    if (lon < -180 || lon > 180) { outputEl.textContent = 'Longitude must be -180 to 180'; return; }
    ...
    outputEl.textContent = 'Date: ' + date.toLocaleDateString() + '\n' +
      'Location: ' + lat.toFixed(4) + '°, ' + lon.toFixed(4) + '°\n\n' +
      '🌅 Sunrise: ' + formatTime(sunrise) + '\n' +
      '🌇 Sunset: ' + formatTime(sunset) + '\n\n' +
      'Daylight Duration: ' + daylightHours.toFixed(1) + ' hours';
