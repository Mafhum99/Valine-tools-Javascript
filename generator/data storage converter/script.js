 ...
 * Convert between different data storage units (binary and decimal)
 */
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Data Storage Converter', icon: '💾' });

    const inputEl = $('#inputValue');
    const systemEl = $('#systemType');
    ...
    function getUnits() {
        return systemEl.value === 'binary' ? binaryUnits : decimalUnits;
    }

    function populateDropdownes() {
        ...
        fromEl.value = keys[0];
        toEl.value = keys[3] || keys[keys.length - 1];
    }

    populateDropdownes();
    systemEl.addEventListener('change', populateDropdownes);
    ...
    function convert() {
        ...
        const systemName = systemEl.value === 'binary' ? 'Binary (IEC)' : 'Decimal (SI)';
        outputEl.innerHTML = `System: ${systemName}\n${formatNumber(val, 6)} ${units[fromEl.value].name} = ${formatNumber(result, 6)} ${units[toEl.value].name}\n\nSteps:\n1. To bytes: ${formatNumber(val, 6)} × ${fromFactor} = ${formatNumber(bytes, 2)} B\n2. To target: ${formatNumber(bytes, 2)} ÷ ${toFactor} = ${formatNumber(result, 6)}\n\nResult: ${formatNumber(result, 6)} ${units[toEl.value].name}`;
    }
