...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Auto Loan Calculator', icon: '🚗' });
    ...
    const carPrice = parseFloat(inputEl.value) || 0;
    const downPayment = parseFloat($('#downPayment').value) || 0;
    const annualRate = parseFloat($('#rate').value) || 0;
    const months = parseInt($('#term').value) || 60;
    
    if (carPrice <= 0) { outputEl.textContent = 'Please enter car price'; return; }
    ...
    outputEl.textContent = 'Car Price: $' + carPrice.toLocaleString() + '\n' +
