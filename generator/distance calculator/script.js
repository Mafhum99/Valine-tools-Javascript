 ...
 * Formulas: d = √[(x₂-x₁)² + (y₂-y₁)²] for 2D
 *           d = √[(x₂-x₁)² + (y₂-y₁)² + (z₂-z₁)²] for 3D
 */

document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Distance Calculator', icon: '📐' });
    ...
    function calculate() {
        ...
        if (is3D) {
            steps += `  Δz = z₂ - z₁ = ${formatNum(z2)} - ${formatNum(z1)} = ${formatNum(dz)}\n`;
        }
        steps += '\n';

        steps += 'Step 2: Square the differences\n';
        steps += `  (Δx)² = (${formatNum(dx)})² = ${formatNum(dx * dx)}\n`;
        steps += `  (Δy)² = (${formatNum(dy)})² = ${formatNum(dy * dy)}\n`;
        if (is3D) {
            steps += `  (Δz)² = (${formatNum(dz)})² = ${formatNum(dz * dz)}\n`;
        }
        steps += '\n';

        steps += 'Step 3: Sum of squares\n';
        const sumSq = dx * dx + dy * dy + (is3D ? dz * dz : 0);
        let sumExpr = `${formatNum(dx * dx)} + ${formatNum(dy * dy)}`;
        if (is3D) sumExpr += ` + ${formatNum(dz * dz)}`;
        steps += `  Sum = ${sumExpr}\n`;
        steps += `  Sum = ${formatNum(sumSq)}\n\n`;

        steps += 'Step 4: Take the square root\n';
        steps += `  d = √${formatNum(sumSq)}\n`;
        steps += `  d = ${formatNum(Math.sqrt(sumSq))}\n\n`;

        steps += 'Result:\n';
        steps += `  Distance = ${formatNum(Math.sqrt(sumSq))} units`;
