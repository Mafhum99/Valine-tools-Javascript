...
/**
 * Port Number Lookup
 * Look up common port numbers
 */

// Initialize tool
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Port Number Lookup', icon: '🔌' });
    ...
    function calculate() {
        ...
        try {
            const ports = {
                ...
                515: 'LPD/LPR', 587: 'SMTP (submission)', 631: 'IPP', 636: 'LDAPS',
            ...
            if (!isNaN(portNum)) {
                const service = ports[portNum] || 'Unknown/Custom';
                const isWellKnown = portNum >= 0 && portNum <= 1023;
                const isRegistered = portNum >= 1024 && portNum <= 49151;
                const isDynamic = portNum >= 49152 && portNum <= 65535;
                let range = isWellKnown ? 'Well-Known' : isRegistered ? 'Registered' : isDynamic ? 'Dynamic/Private' : 'Invalid';
                outputEl.textContent = `Port: ${portNum}\nService: ${service}\nRange: ${range}`;
            } else {
                const results = [];
                ...
                if (results.length === 0) throw new Error(`No port found for "${input}"`);
