...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'MIME Types Reference', icon: '📁' });
    ...
    function calculate() {
        ...
        try {
            ...
            const mimes = [
                ...
                { ext: 'xml', mime: 'application/xml', desc: 'XML document' },
                { ext: 'txt', mime: 'text/plain', desc: 'Plain text' },
                { ext: 'csv', mime: 'text/csv', desc: 'CSV data' },
                ...
                { ext: 'form', mime: 'application/x-www-form-urlencoded', desc: 'Form data' },
                { ext: 'multipart', mime: 'multipart/form-data', desc: 'Multipart form data' },
                { ext: 'stream', mime: 'application/octet-stream', desc: 'Binary stream (fallback)' },
                { ext: 'manifest', mime: 'text/cache-manifest', desc: 'AppCache manifest' },
                { ext: 'webmanifest', mime: 'application/manifest+json', desc: 'Web app manifest' },
                { ext: 'md', mime: 'text/markdown', desc: 'Markdown' },
                { ext: 'yaml', mime: 'application/x-yaml', desc: 'YAML' },
                { ext: 'yml', mime: 'application/x-yaml', desc: 'YAML' }
            ];
            const results = query ? mimes.filter(m => m.ext.includes(query) || m.mime.toLowerCase().includes(query) || m.desc.toLowerCase().includes(query)) : mimes;
            if (results.length === 0) throw new Error('No matching MIME type found');
            outputEl.textContent = results.map(m => `${m.ext.padEnd(15)} ${m.mime}\n${' '.repeat(15)} ${m.desc}`).join('\n\n');
        } catch (error) {
