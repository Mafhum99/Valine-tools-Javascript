...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'JSON Formatter & Validator', icon: '\u{1F4CB}' });
    ...
    function validateJSON() {
        try {
            ...
            outputEl.textContent = '\u2705 Valid JSON\nType: ' + type + '\n' +
                (typeof obj === 'object' && obj !== null ? 'Top-level keys: ' + keys + '\n' : '') +
                'Size: ' + new Blob([JSON.stringify(obj)]).size + ' bytes';
        } catch (e) {
            outputEl.textContent = '\u274C Invalid JSON: ' + e.message;
        }
    }
