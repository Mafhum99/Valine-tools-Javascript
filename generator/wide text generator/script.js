...
/**
 * 543 - Wide Text Generator
 * Generates text using fullwidth Unicode characters
 */

document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Wide Text Generator', icon: 'Ｗ' });
    ...
    // Extra wide with spaces
    function toExtraWide(text) {
        return text.split('').map(char => {
            const code = char.charCodeAt(0);
            if (code >= 0x20 && code <= 0x7E) {
                return String.fromCodePoint(code + 0xFEE0);
    ...
    function calculate() {
        ...
        try {
            let output = '═══ Wide Text ═══\n\n';
            output += toWide(input);
            output += '\n\n═══ Extra Wide (spaced) ═══\n\n';
            output += toExtraWide(input);
            output += '\n\n═══ Original ═══\n\n';
            output += input;
