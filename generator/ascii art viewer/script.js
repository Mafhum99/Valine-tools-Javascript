...
/**
 * ASCII Art Viewer
 * Display and preview ASCII art with monospace font
 */

document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'ASCII Art Viewer', icon: '\uD83C\uDFA8' });
...
`
    ...
    function render() {
        ...
        if (!input.trim()) {
            outputEl.textContent = 'Please paste some ASCII art';
            return;
        }
        try {
    ...
    inputEl.addEventListener('input', debounce(render, 300));

    $('#sample1').addEventListener('click', () => {
        inputEl.value = samples.cat;
        render();
    });

    $('#sample2').addEventListener('click', () => {
        inputEl.value = samples.dog;
        render();
    });

    $('#sample3').addEventListener('click', () => {
        inputEl.value = samples.tree;
        render();
    });

    $('#sample4').addEventListener('click', () => {
        inputEl.value = samples.rocket;
        render();
    });
});
