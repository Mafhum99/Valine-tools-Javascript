...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Case Converter', icon: '🔠' });
    ...
    function toSnakeCase(str) {
        return str.match(/[A-Z]{2,}(?=[A-Z][a-z]+[0-9]*|\b)|[A-Z]?[a-z]+[0-9]*|[A-Z]|[0-9]+/g)
            ?.join('_').toLowerCase() || str.toLowerCase();
    }
    ...
    function convert() {
        ...
        try {
            const cases = [
                ...
                { name: 'Title Case', value: toTitleCase(input) },
                { name: 'Sentence case', value: toSentenceCase(input) },
                ...
                { name: 'CONSTANT_CASE', value: toConstantCase(input) },
                { name: 'tOGGLE cASE', value: toToggleCase(input) },
            ];
            ...
            outputEl.querySelectorAll('.copy-case-btn').forEach(btn => {
    ...
    if (copyBtn) {
        copyBtn.addEventListener('click', () => {
            ...
            copyToClipboard(input.toUpperCase());
        });
    }
});
