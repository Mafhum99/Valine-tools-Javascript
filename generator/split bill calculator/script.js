...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Split Bill Calculator', icon: '🧾' });
    ...
    const bill = parseFloat(inputEl.value) || 0;
    const taxPercent = parseFloat($('#taxPercent').value) || 0;
    const tipPercent = parseFloat($('#tipPercent').value) || 0;
    const numPeople = parseInt($('#numPeople').value) || 1;
    
    if (bill <= 0) { outputEl.textContent = 'Please enter bill amount'; return; }
    if (numPeople < 1) { outputEl.textContent = 'Must be at least 1 person'; return; }
    ...
    outputEl.textContent = 'Bill: $' + bill.toFixed(2) + '\n' +
      'Tax (' + taxPercent + '%): $' + tax.toFixed(2) + '\n' +
