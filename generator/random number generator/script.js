...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function generateRandomInt(min, max) {
        ...
        if (typeof crypto !== 'undefined' && crypto.getRandomValues) {
            ...
            do {
                ...
                value = value & mask;
            } while (value >= range); // Reject values that would cause bias
    ...
    function generate() {
        ...
        if (isNaN(min) || isNaN(max)) {
            outputEl.textContent = 'Please enter valid numbers for min and max';
            return;
        }

        if (min > max) {
            outputEl.textContent = 'Minimum must be less than or equal to maximum';
            return;
        }
        ...
        try {
            ...
            for (let i = 0; i < count; i++) {
                numbers.push(generateRandomInt(min, max));
            }
