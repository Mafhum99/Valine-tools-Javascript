 ...
 * Analyze text for character count, word count, sentence count, and more
 */

document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Text Statistics Analyzer', icon: '📊' });
    ...
    function calculate() {
        ...
        if (!input.trim()) { outputEl.textContent = 'Please enter text to analyze'; return; }
        try {
            ...
            const readingTimeMin = words > 0 ? Math.ceil(words / 200) : 0; // 200 WPM
            const speakingTimeMin = words > 0 ? Math.ceil(words / 130) : 0; // 130 WPM
