 ...
 * Convert between different length/ distance units
 */
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Length Unit Converter', icon: '📏' });
    ...
    const units = {
        ...
        'micrometer': { name: 'Micrometer (μm)', factor: 1e-6 },
        ...
        'light_year': { name: 'Light Year (ly)', factor: 9.461e15 }
    };
    ...
    function convert() {
        ...
        const fromName = units[fromEl.value].name;
        const toName = units[toEl.value].name;

        // Calculate in meters as intermediate
        const meters = val * fromFactor;

        outputEl.innerHTML = `${formatNumber(val, 6)} ${fromName} = ${formatNumber(result, 6)} ${toName}\n\nSteps:\n1. Convert to meters: ${formatNumber(val, 6)} × ${fromFactor} = ${formatNumber(meters, 6)} m\n2. Convert to target: ${formatNumber(meters, 6)} ÷ ${toFactor} = ${formatNumber(result, 6)}\n\nResult: ${formatNumber(result, 6)} ${toName}`;
    }
