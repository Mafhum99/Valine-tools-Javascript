...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Time Zone Converter', icon: '🌍' });
    ...
    if (!dateStr) { outputEl.textContent = 'Please select date and time'; return; }
    
    const fromTZ = $('#fromTZ').value;
    const toTZ = $('#toTZ').value;
    
    const date = new Date(dateStr);
    if (isNaN(date)) { outputEl.textContent = 'Invalid date'; return; }
    ...
    outputEl.textContent = 'Original (' + fromTZ + '):\n' + date.toLocaleString('en-US') + '\n\n' +
      'Converted (' + toTZ + '):\n' + converted;
