...
/**
 * Invoice Template Generator - 603
 * Generate invoice templates
 */

document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Invoice Template Generator', icon: '🧾' });
    ...
    function generateInvoice(description) {
        ...
        if (items.length === 1) {
            items.push(`Service - ${items[0]}`, 'Processing Fee');
        }
        ...
        invoice += `${'-'.repeat(50)}\n`;
        invoice += `ITEMIZED CHARGES:\n`;
        invoice += `${'-'.repeat(50)}\n`;

        let subtotal = 0;
        items.forEach((item, i) => {
            const qty = randomInt(1, 10);
            const rate = randomFloat(25, 500);
            const amount = qty * rate;
            subtotal += amount;
            invoice += `\n${i + 1}. ${item}\n`;
            invoice += `   Qty: ${qty} x $${rate.toFixed(2)} = $${amount.toFixed(2)}\n`;
        });
        ...
        invoice += `Payment Methods: Bank Transfer, Check, or Online Payment\n\n`;
        invoice += `Notes: Thank you for your business! Please remit payment by the due date.\n\n`;
        invoice += `End of Invoice - ${invoiceNum}\n`;
    ...
    function calculate() {
        ...
        if (!input) {
            outputEl.textContent = 'Please enter invoice description (comma-separated items)';
            return;
        }
        try {
            outputEl.textContent = generateInvoice(input);
        } catch (error) {
