...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Paragraph Counter', icon: '📋' });
    ...
    function count() {
        ...
        try {
            ...
            if (paragraphs.length === 0) {
                outputEl.textContent = 'No paragraphs found';
                return;
            }

            let html = '<div class="paragraph-list">';
            paragraphs.forEach((p, i) => {
                html += `<div class="paragraph-item"><span class="paragraph-number">${i + 1}</span><p class="paragraph-text">${p}</p></div>`;
            });
            html += '</div>';
    ...
    if (copyBtn) {
        copyBtn.addEventListener('click', () => {
            ...
            const paragraphs = input
                ...
                .map(p => p.trim())
                .filter(p => p.length > 0);
            const text = `Paragraph Count: ${paragraphs.length}\n\n${paragraphs.map((p, i) => `--- Paragraph ${i + 1} ---\n${p}`).join('\n\n')}`;
