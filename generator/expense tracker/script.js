...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Expense Tracker', icon: '📝' });
    ...
    if (!description || amount <= 0) {
      outputEl.textContent = 'Please enter description and amount';
      return;
    }
    ...
    let result = '💰 Total Expenses: $' + total.toFixed(2) + '\n\n';
    expenses.forEach((exp, idx) => {
      result += (idx + 1) + '. ' + exp.description + ': $' + exp.amount.toFixed(2) + ' (' + exp.date + ')\n';
    });
    
    result += '\nTo clear all expenses, use the Clear button';
