...
/**
 * 544 - Mirror Text Generator
 * Generates mirrored/reversed text
 */

document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Mirror Text Generator', icon: '🪞' });
    ...
    function mirrorText(text) {
        // Reverse and mirror each character
        return text
            .split('')
            .reverse()
    ...
    // Just reverse without character mapping
    function reverseText(text) {
        return text.split('').reverse().join('');
    }

    // Character mirror without reversing
    function charMirror(text) {
    ...
    function calculate() {
        ...
        try {
            let output = '═══ Mirror Text (reversed + mirrored) ═══\n\n';
            output += mirrorText(input);
            output += '\n\n═══ Reverse Only ═══\n\n';
            output += reverseText(input);
            output += '\n\n═══ Character Mirror (no reverse) ═══\n\n';
            output += charMirror(input);
