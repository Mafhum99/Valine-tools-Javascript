...
/**
 * 521 - Words to Number Converter
 * Converts written words to numbers
 */

document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Words to Number Converter', icon: '🔢' });
    ...
    function wordsToNumber(text) {
        ...
        const filteredWords = words.filter(w => {
            if (w === 'negative') { isNegative = true; return false; }
            return true;
        });

        const scales = { 'hundred': 100, 'thousand': 1000, 'million': 1000000, 'billion': 1000000000, 'trillion': 1000000000000 };
    ...
    function calculate() {
        ...
        if (!input) { outputEl.textContent = 'Please enter words (e.g., "one hundred twenty-three")'; return; }

        const num = wordsToNumber(input);
        if (num === null) {
            outputEl.textContent = 'Could not parse. Use words like "one hundred twenty-three"';
            return;
        }
