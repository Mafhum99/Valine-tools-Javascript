...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'XML to JSON Converter', icon: '\u{1F504}' });
        ...
        function nodeToJson(node) {
            ...
            if (node.attributes && node.attributes.length > 0) {
                ...
                for (let i = 0; i < node.attributes.length; i++) {
                    ...
                    attrs['@' + attr.name] = parseValue(attr.value);
                }
            ...
            if (elementChildren.length > 0) {
                ...
                for (const child of elementChildren) {
                    if (!groups[child.tagName]) {
                        groups[child.tagName] = [];
                    }
                    groups[child.tagName].push(nodeToJson(child));
                }
                ...
                // Has attributes but no element children
                const trimmed = textContent.trim();
                if (trimmed) {
                    result['#text'] = parseValue(trimmed);
                }
            }
    ...
    function convert() {
        ...
        if (!raw) {
            outputEl.textContent = 'Please enter XML to convert.';
            return;
        }
        try {
            const obj = xmlToJson(raw);
            outputEl.textContent = JSON.stringify(obj, null, 2);
        } catch (e) {
            outputEl.textContent = '\u274C XML parse error: ' + e.message;
        }
    }
