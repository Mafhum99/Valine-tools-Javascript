...
/**
 * 520 - Number to Words Converter
 * Converts numbers to written words
 */

document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Number to Words Converter', icon: '🔢' });
    ...
    function toOrdinal(num) {
        ...
        // Simple ordinal conversion for common cases
        ...
        if (ordinalMap[lastWord]) {
            result[result.length - 1] = ordinalMap[lastWord];
        } else if (lastWord.endsWith('y')) {
            result[result.length - 1] = lastWord.slice(0, -1) + 'ieth';
        } else if (lastWord.includes('-')) {
            const parts = lastWord.split('-');
            const lastPart = parts[parts.length - 1];
            if (ordinalMap[lastPart]) {
            ...
            result[result.length - 1] = lastWord + 'th';
        }
    ...
    function calculate() {
        ...
        if (!input || isNaN(num)) { outputEl.textContent = 'Please enter a valid number'; return; }
        ...
        if (input.includes('.')) {
            ...
            const c = parseInt(cents.padEnd(2, '0').substring(0, 2), 10);
            result += `${numberToWords(d)} dollars and ${numberToWords(c)} cents`;
        } else {
            result += `${numberToWords(num)} dollars`;
        }
