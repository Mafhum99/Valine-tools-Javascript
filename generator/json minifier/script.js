...
/**
 * JSON Minifier
 * Remove all whitespace from JSON to reduce file size
 */

document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'JSON Minifier', icon: '\u{1F4E6}' });
    ...
    function minify() {
        ...
        if (!raw) {
            outputEl.textContent = 'Please enter JSON to minify';
            return;
        }
        try {
    ...
    if (copyBtn) {
        copyBtn.addEventListener('click', () => {
            const minified = outputEl.textContent.split('\n\n---')[0];
