...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Subnet Calculator', icon: '🌐' });
    ...
    function calculate() {
        ...
        try {
            ...
            if (!match) throw new Error('Format: IP/CIDR (e.g., 192.168.1.0/24)');
            const [, ip, cidrStr] = match;
            const cidr = parseInt(cidrStr);
            if (cidr < 0 || cidr > 32) throw new Error('CIDR must be 0-32');
            const octets = ip.split('.').map(Number);
            if (octets.some(o => o > 255)) throw new Error('Invalid IP address');
            ...
            let result = `Network: ${toIP(network)}/${cidr}\n`;
            result += `Netmask: ${maskIP}\n`;
            result += `Wildcard: ${wildCard}\n`;
            result += `Broadcast: ${toIP(broadcast)}\n`;
            if (cidr < 31) {
                result += `First Host: ${toIP(firstHost)}\n`;
                result += `Last Host: ${toIP(lastHost)}\n`;
                result += `Usable Hosts: ${totalHosts.toLocaleString()}\n`;
            } else {
                result += `Total Addresses: ${totalHosts}\n`;
            }
            result += `Total IPs: ${Math.pow(2, 32 - cidr).toLocaleString()}`;
