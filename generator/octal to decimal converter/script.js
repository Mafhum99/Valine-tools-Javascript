...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Octal to Decimal Converter', icon: '🔵' });

    const inputEl = $('#octalInput');
    ...
    function convert() {
        let val = inputEl.value.trim().replace(/^0o/i, '');
        if (!val) {
            outputEl.textContent = 'Please enter an octal number';
            return;
        }
        if (!/^[0-7]+$/.test(val)) {
            outputEl.textContent = 'Error: Invalid octal. Use only digits 0-7.';
            return;
        }
        ...
        digits.forEach((digit, i) => {
            const p = power - i;
            const term = parseInt(digit) * Math.pow(8, p);
            steps += `${digit} × 8^${p} = ${parseInt(digit)} × ${Math.pow(8, p)} = ${term}\n`;
        });
        steps += `\nSum = ${decimal}`;
