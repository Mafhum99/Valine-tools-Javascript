...
/**
 * Sentence Counter
 * Count and list sentences in text
 */

document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Sentence Counter', icon: '📄' });
    ...
    function count() {
        ...
        try {
            ...
            if (sentences.length === 0) {
                outputEl.textContent = 'No sentences found';
                return;
            }

            let html = '<ol class="sentence-list">';
    ...
    if (copyBtn) {
        copyBtn.addEventListener('click', () => {
            ...
            const sentences = input
                ...
                .map(s => s.trim())
                .filter(s => s.length > 0);
            const text = `Sentence Count: ${sentences.length}\n\n${sentences.map((s, i) => `${i + 1}. ${s}`).join('\n')}`;
