...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Text Merger', icon: '\u{1F517}' });

    const text1El = $('#text1');
    const text2El = $('#text2');
    const separatorEl = $('#separator');
    const customSeparatorGroup = $('#custom-separator-group');
    const customSeparatorEl = $('#custom-separator');
    ...
    function getSeparator() {
        const val = separatorEl.value;
        if (val === 'custom') {
            return customSeparatorEl.value || '';
        }
        return val.replace(/\\n/g, '\n');
    }
    ...
    function merge() {
        ...
        if (!text1 && !text2) {
            outputEl.textContent = 'Please enter at least one text';
            return;
        }
