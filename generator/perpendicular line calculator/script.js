 ...
 * Key property: m_perp = -1/m (negative reciprocal)
 */

document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Perpendicular Line Calculator', icon: '📐' });
    ...
    function buildPerpendicularLineResult(origM, origLineStr, isOrigVertical, px, py, origB) {
        let steps = '';

        steps += 'Original line:\n';
        steps += `  ${origLineStr}\n`;
        steps += `  Original slope (m): ${isOrigVertical ? 'undefined (vertical)' : formatNum(origM)}\n\n`;

        steps += `Point for perpendicular line: (${formatNum(px)}, ${formatNum(py)})\n\n`;

        steps += 'Key property: Perpendicular lines have negative reciprocal slopes\n';

        if (isOrigVertical) {
            // Original line is vertical (x = const), perpendicular is horizontal (y = const)
            steps += '  Original line is VERTICAL → Perpendicular line is HORIZONTAL\n\n';
            steps += 'Result:\n';
            steps += `  Perpendicular line equation: y = ${formatNum(py)}\n`;
            steps += `  Slope-intercept: y = ${formatNum(py)}\n`;
            steps += `  Standard form: y = ${formatNum(py)}\n`;
            steps += `  This is a horizontal line through y = ${formatNum(py)}`;
            return steps;
        }

        if (origM === 0) {
            // Original line is horizontal, perpendicular is vertical
            steps += '  Original line is HORIZONTAL (m = 0) → Perpendicular line is VERTICAL\n\n';
            steps += 'Result:\n';
            steps += `  Perpendicular line equation: x = ${formatNum(px)}\n`;
            steps += `  This is a vertical line through x = ${formatNum(px)}`;
            return steps;
        }

        const perpM = -1 / origM;

        steps += `  m_perp = -1/m = -1/(${formatNum(origM)})\n`;
        steps += `  m_perp = ${formatNum(perpM)}\n\n`;

        // Verify: m * m_perp should equal -1
        steps += 'Verification:\n';
        steps += `  m × m_perp = ${formatNum(origM)} × ${formatNum(perpM)} = ${formatNum(origM * perpM)}\n\n`;

        const newB = py - perpM * px;

        steps += 'Calculate y-intercept of perpendicular line:\n';
        steps += `  b = y₀ - m_perp × x₀\n`;
        steps += `  b = ${formatNum(py)} - ${formatNum(perpM)} × ${formatNum(px)}\n`;
        steps += `  b = ${formatNum(py)} - ${formatNum(perpM * px)}\n`;
        ...
        if (perpM === 0) {
            ...
            steps += `  Slope-intercept: y = ${formatNum(perpM)}x\n`;
        } else {
            steps += `  Slope-intercept: y = ${formatNum(perpM)}x ${formatSignedNum(newB)}\n`;
        }

        // Point-slope form
        steps += `  Point-slope: y - ${formatNum(py)} = ${formatNum(perpM)}(x - ${formatNum(px)})\n`;

        // Standard form
        const mFrac = toFraction(perpM);
        const bFrac = toFraction(newB);
        if (mFrac && bFrac) {
            const lcm = lcmOf(mFrac.den, bFrac.den);
            let A = Math.round(-perpM * lcm);
    ...
    function calculate() {
        ...
        let origM, origLineStr, isOrigVertical = false, origB = 0;

        switch (lineInputModeEl.value) {
            case 'slopeintercept': {
                origM = parseFloat(origSlopeEl.value);
                const b = parseFloat(origInterceptEl.value);
                if (isNaN(origM)) {
                    outputEl.textContent = 'Error: Please enter a valid slope.';
                    return;
                }
                origB = isNaN(b) ? 0 : b;
                origLineStr = isNaN(b)
                    ? `y = ${formatNum(origM)}x + b (b unknown)`
                    : `y = ${formatNum(origM)}x ${formatSignedNum(b)}`;
                break;
            }
            case 'standard': {
        ...
        outputEl.textContent = buildPerpendicularLineResult(origM, origLineStr, isOrigVertical, px, py, origB);
    }
