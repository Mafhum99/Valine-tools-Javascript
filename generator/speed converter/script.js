...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Speed Converter', icon: '💨' });
    ...
    const units = {
        ...
        'mach': { name: 'Mach (at sea level)', factor: 340.29 },
        'speed_of_light': { name: 'Speed of Light (c)', factor: 299792458 }
    };
    ...
    function convert() {
        ...
        const mps = val * fromFactor;
        const result = mps / toFactor;

        outputEl.innerHTML = `${formatNumber(val, 6)} ${units[fromEl.value].name} = ${formatNumber(result, 6)} ${units[toEl.value].name}\n\nSteps:\n1. To m/s: ${formatNumber(val, 6)} × ${fromFactor} = ${formatNumber(mps, 6)} m/s\n2. To target: ${formatNumber(mps, 6)} ÷ ${toFactor} = ${formatNumber(result, 6)}\n\nResult: ${formatNumber(result, 6)} ${units[toEl.value].name}`;
    }
