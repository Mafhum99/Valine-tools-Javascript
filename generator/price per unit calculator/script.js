...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Price Per Unit Calculator', icon: '🧮' });
    ...
    function calculate() {
        try {
            ...
            if (quantity === 0) {
                outputEl.textContent = 'Error: Quantity cannot be zero';
                return;
            }
            const result = totalPrice / quantity;
            outputEl.textContent = 'Price Per Unit: $' + result.toFixed(2);
        } catch (error) {
    ...
    [totalPriceEl, quantityEl].forEach(el => el.addEventListener('keypress', (e) => { if (e.key === 'Enter') calculate(); }));
});
