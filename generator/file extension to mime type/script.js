...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'File Extension to MIME Type', icon: '📄' });
    ...
    function calculate() {
        ...
        try {
            const mimes = {
                ...
                'flac': 'audio/flac', 'aac': 'audio/aac', 'midi': 'audio/midi',
                ...
                'rtf': 'application/rtf', 'exe': 'application/octet-stream', 'dmg': 'application/x-apple-diskimage',
                'iso': 'application/x-iso9660-image', 'sql': 'application/sql', 'sh': 'application/x-sh',
            ...
            if (mimes[ext]) {
                outputEl.textContent = `Extension: .${ext}\nMIME Type: ${mimes[ext]}\n\nUsage:\nContent-Type: ${mimes[ext]}\n\nHTML:\n<link rel="..." type="${mimes[ext]}">\n<img src="file.${ext}" type="${mimes[ext]}">`;
            } else {
                const results = [];
                Object.entries(mimes).forEach(([e, m]) => {
                    if (e.includes(ext) || m.includes(ext)) results.push(`.${e} → ${m}`);
                });
                if (results.length === 0) throw new Error(`No MIME type found for .${ext}`);
