...
document.addEventListener('DOMContentLoaded', () => {
    initTool({ name: 'Hexadecimal to Decimal Converter', icon: '🔷' });

    const inputEl = $('#hexInput');
    ...
    function convert() {
        let val = inputEl.value.trim().replace(/^0x/i, '');
        if (!val) {
            outputEl.textContent = 'Please enter a hexadecimal number';
            return;
        }
        if (!/^[0-9A-Fa-f]+$/.test(val)) {
            outputEl.textContent = 'Error: Invalid hexadecimal. Use only 0-9 and A-F.';
            return;
        }

        const decimal = parseInt(val, 16);
        if (isNaN(decimal)) {
            outputEl.textContent = 'Error: Conversion failed.';
            return;
        }

        let steps = `Hexadecimal: ${val}\nDecimal: ${decimal}\n\nCalculation:\n`;
        const digits = val.toUpperCase().split('');
        const power = digits.length - 1;
        digits.forEach((digit, i) => {
            const decVal = parseInt(digit, 16);
            const p = power - i;
            steps += `${digit} × 16^${p} = ${decVal} × ${Math.pow(16, p)} = ${decVal * Math.pow(16, p)}\n`;
        });
        steps += `\nSum = ${decimal}`;
